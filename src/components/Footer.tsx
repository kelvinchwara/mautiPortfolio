...
import {
  ...
  GraduationCap, Briefcase, Code, Coffee, Rocket, Palette
} from 'lucide-react';
...
const FooterLink: React.FC<FooterLinkProps> = ({ to, icon: Icon, children }) => (
  <Link 
    to={to}
    className="flex items-center gap-2 text-gray-100 hover:text-white transition-all duration-300 group py-1"
  >
    <Icon className="h-4 w-4 group-hover:translate-x-1 transition-transform duration-300" />
    <span className="group-hover:translate-x-1 transition-transform duration-300">
      {children}
    </span>
    <ArrowRight className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-all duration-300 -ml-2 group-hover:ml-0" />
  </Link>
);
...
const SocialButton: React.FC<SocialButtonProps> = ({ icon: Icon, href, label }) => (
  <a
    ...
    title={label}
  >
    <div className="p-3 rounded-full bg-white/10 hover:bg-white transition-all duration-300 hover:scale-110 hover:rotate-6 shadow-lg">
...
const StatCard: React.FC<StatCardProps> = ({ icon: Icon, value, label }) => (
  <div className="text-center p-4 rounded-xl bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-all duration-300 hover:scale-105 group">
  ...
  <div className="lg:col-span-1">
    ...
    <div className="flex flex-wrap gap-2 mb-4">
      {FOOTER_DATA.profile.badges.map((badge, idx) => (
        <div 
          key={idx}
          className="px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm text-xs font-medium text-white flex items-center gap-1"
        >
  ...
  <div>
    ...
    <div className="mt-6">
      <a
        ...
        download
        className="flex items-center gap-2 px-4 py-2 bg-white text-gray-800 rounded-lg hover:bg-gray-100 transition-all duration-300 hover:scale-105 shadow-lg text-sm font-semibold group"
      >
